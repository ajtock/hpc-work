...
# ./get_segment_pairs_alnToSame.R 'Chr1,Chr2,Chr3,Chr4,Chr5' 10000 0.95 Col-0.ragtag_scaffolds Ler-0_110x.ragtag_scaffolds Col-0.ragtag_scaffolds_Chr co 241022
# conda deactivate
...
#not_centromere/Chr1/segments/Col-0/co/ff7fb804-706d-45db-bf28-284ff423dbf5__Col-0_alnTo_Col-0.ragtag_scaffolds_Chr_mm_ont.paf

# wm alignments
acc1_wm = dplyr::bind_rows(
    mclapply(1:length(acc1_indir_list), function(x) {
...
acc2_wm = dplyr::bind_rows(
    mclapply(1:length(acc2_indir_list), function(x) {
...
#[1] "716 putative co events are between homologous chromosomes"
...
#[1] "21.9% of putative co events are between homologous chromosomes"
...
#[1] "20 putative co events are between homologous chromosomes where the per-accession read segments align to within 10000 bp of each other in the same reference assembly"
...
#[1] "0.6% of putative co events are between homologous chromosomes where the per-accession read segments align to within 10000 bp of each other in the same reference assembly"
...
#[1] "2.79% of putative co events that are between homologous chromosomes are those where the per-accession read segments align to within 10000 bp of each other in the same reference assembly"
...
#[1] "5 putative co events are between homologous chromosomes where the per-accession read segments align to within 10000 bp of each other in the same reference assembly, and where the per-accession alignment length is >= 95% of the segment length"
...
#[1] "0.15% of putative co events are between homologous chromosomes where the per-accession read segments align to within 10000 bp of each other in the same reference assembly, and where the per-accession alignment length is >= 95% of the segment length"
...
#[1] "25% of putative co events that are between homologous chromosomes and where the per-accession read segments align to within 10000 bp of each other in the same reference assembly, are those where the per-accession alignment length is >= 95% of the segment length"
...
aln_best_pair_DF <- read.table(paste0(outDir,
                                      acc1, "_", acc2,
                                      "_putative_", recombType, "_best_segment_pairs",
                                      "_alnTo_", alnTo, "_",
                                      paste0(chrName, collapse = "_"), "_v", date, ".tsv"),
                               header = T)
aln_best_pair_hom_DF <- read.table(paste0(outDir,
                                          acc1, "_", acc2,
                                          "_putative_homologous_", recombType, "_best_segment_pairs",
                                          "_alnTo_", alnTo, "_",
                                          paste0(chrName, collapse = "_"), "_v", date, ".tsv"),
